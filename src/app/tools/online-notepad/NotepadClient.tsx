...
export default function NotepadClient() {
  ...
  async function savePDF() {
    ...
    try {
      ...
      const canvas = await html2canvas(el, { scale: 2, useCORS: true, backgroundColor: "#ffffff", scrollX: 0, scrollY: 0, windowWidth: el.scrollWidth, windowHeight: el.scrollHeight, width: el.scrollWidth, height: el.scrollHeight });
      const imgData  = canvas.toDataURL("image/png");
      const pdfW = 210, pdfH = 297, margin = 12;
      const usableW = pdfW - margin * 2;
      const ratio = usableW / (canvas.width / 3.7795);
      const scaledH = (canvas.height / 3.7795) * ratio;
      const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
      let yPos = margin, remaining = scaledH;
      while (remaining > 0) {
  ...
  function toggleDiff() {
    if (!showDiff) setDiffSnapshot(editor?.getText() ?? "");
    setShowDiff(d => !d);
      ...
      <div style={{ minHeight:"100vh", background:bg, color:text_c, fontFamily:"system-ui,sans-serif", display:"flex", flexDirection:"column", margin:"0 auto", boxShadow:"0 0 12px rgba(0,0,0,0.08)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"7px 20px", background:surface, borderBottom:`1px solid ${border}`, flexShrink:0 }}>
          ...
          <div style={{ marginLeft:"auto", display:"flex", gap:8, alignItems:"center" }}>
            ...
            <button onClick={()=>setDarkMode(d=>!d)} style={{ padding:"5px 14px", fontSize:12, fontWeight:600, border:`1px solid ${border}`, borderRadius:6, background:darkMode?"#fff":"#1a1a1a", color:darkMode?"#000":"#fff", cursor:"pointer" }}>
              {darkMode?"☀":"🌙"}
            </button>
          </div>
        </div>
        ...
        <div className="color-root" style={{ display:"flex", alignItems:"center", gap:3, padding:"5px 20px", background:surface, borderBottom:`1px solid ${border}`, flexWrap:"wrap", flexShrink:0 }}>
          <button onClick={()=>editor?.chain().focus().toggleBold().run()} style={{...BStyle(editor?.isActive("bold")), fontWeight:700, width:28, textAlign:"center"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=editor?.isActive("bold")?hover:"transparent")}>B</button>
          <button onClick={()=>editor?.chain().focus().toggleItalic().run()} style={{...BStyle(editor?.isActive("italic")), fontStyle:"italic", width:28, textAlign:"center"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=editor?.isActive("italic")?hover:"transparent")}>I</button>
          <button onClick={()=>editor?.chain().focus().toggleUnderline?.()?.run()} style={{...BStyle(editor?.isActive("underline")), textDecoration:"underline", width:28, textAlign:"center"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background="transparent")}>U</button>
          <button onClick={()=>editor?.chain().focus().toggleStrike().run()} style={{...BStyle(editor?.isActive("strike")), textDecoration:"line-through", width:28, textAlign:"center"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=editor?.isActive("strike")?hover:"transparent")}>S</button>
          <button onClick={()=>editor?.chain().focus().toggleCode().run()} style={{...BStyle(editor?.isActive("code")), fontFamily:"monospace", width:28, textAlign:"center", fontSize:11}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=editor?.isActive("code")?hover:"transparent")}>{`</>`}</button>
          <div style={{width:1,height:22,background:border,margin:"0 5px"}}/>
          {([1,2,3] as const).map(l=>(
            <button key={l} onClick={()=>editor?.chain().focus().toggleHeading({level:l}).run()} style={{...BStyle(editor?.isActive("heading",{level:l})), fontSize:11, padding:"3px 8px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=editor?.isActive("heading",{level:l})?hover:"transparent")}>H{l}</button>
          ))}
          <div style={{width:1,height:22,background:border,margin:"0 5px"}}/>
          <button onClick={()=>editor?.chain().focus().toggleBulletList().run()} style={{...BStyle(editor?.isActive("bulletList")), fontSize:13, padding:"3px 8px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=editor?.isActive("bulletList")?hover:"transparent")}>• List</button>
          <button onClick={()=>editor?.chain().focus().toggleOrderedList().run()} style={{...BStyle(editor?.isActive("orderedList")), fontSize:13, padding:"3px 8px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=editor?.isActive("orderedList")?hover:"transparent")}>1. List</button>
          <button onClick={()=>editor?.chain().focus().toggleBlockquote().run()} style={{...BStyle(editor?.isActive("blockquote")), fontSize:13, padding:"3px 8px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=editor?.isActive("blockquote")?hover:"transparent")}>❝</button>
          <div style={{width:1,height:22,background:border,margin:"0 5px"}}/>
          {(["left","center","right"] as const).map(dir=>(
            <button key={dir} onClick={()=>editor?.chain().focus().setTextAlign(dir).run()} style={{...BStyle(editor?.isActive({textAlign:dir})), fontSize:12, padding:"3px 10px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=editor?.isActive({textAlign:dir})?hover:"transparent")}>
          ...
          <div style={{position:"relative"}}>
            <button onClick={e=>{ e.stopPropagation(); setShowTC(v=>!v); setShowHC(false); }} style={{...BStyle(), display:"flex", alignItems:"center", gap:5, padding:"3px 9px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background="transparent")}>
            ...
            {showTC && (
              <div style={{ position:"absolute", top:"110%", left:0, background:menuBg, border:`1px solid ${border}`, borderRadius:8, padding:12, zIndex:300, boxShadow:"0 8px 24px rgba(0,0,0,0.14)", minWidth:200 }}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8}}>
                  {TCOLORS.map(c=>(<button key={c} onClick={()=>{ editor?.chain().focus().setColor(c).run(); setCurTC(c); setShowTC(false); }} style={{width:26,height:26,borderRadius:5,background:c,border:c===curTC?`2px solid #1a1a1a`:`1px solid ${border}`,cursor:"pointer",boxShadow:c==="#ffffff"?"inset 0 0 0 1px #ccc":"none"}}/>))}
                </div>
                <input type="color" value={curTC} onChange={e=>{ setCurTC(e.target.value); editor?.chain().focus().setColor(e.target.value).run(); }} style={{width:"100%",height:28,border:`1px solid ${border}`,borderRadius:5,cursor:"pointer",background:"transparent"}}/>
                <button onClick={()=>{ editor?.chain().focus().unsetColor().run(); setCurTC("#000000"); setShowTC(false); }} style={{marginTop:6,fontSize:11,color:muted,background:"transparent",border:"none",cursor:"pointer"}}>✕ Remove color</button>
              </div>
            )}
          </div>
          {/* Highlight color */}
          <div style={{position:"relative"}}>
            <button onClick={e=>{ e.stopPropagation(); setShowHC(v=>!v); setShowTC(false); }} style={{...BStyle(editor?.isActive("highlight")), display:"flex", alignItems:"center", gap:5, padding:"3px 9px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=editor?.isActive("highlight")?hover:"transparent")}>
            ...
            {showHC && (
              <div style={{ position:"absolute", top:"110%", left:0, background:menuBg, border:`1px solid ${border}`, borderRadius:8, padding:12, zIndex:300, boxShadow:"0 8px 24px rgba(0,0,0,0.14)", minWidth:200 }}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {HCOLORS.map(c=>(<button key={c} onClick={()=>{ if(c==="none"){ editor?.chain().focus().unsetHighlight().run(); } else { editor?.chain().focus().setHighlight({color:c}).run(); } setCurHC(c); setShowHC(false); }} style={{width:26,height:26,borderRadius:5,background:c==="none"?"repeating-linear-gradient(45deg,#ccc 0,#ccc 2px,#fff 0,#fff 6px)":c,border:c===curHC?`2px solid #1a1a1a`:`1px solid ${border}`,cursor:"pointer"}}/>))}
                </div>
              </div>
            )}
          </div>
          <div style={{width:1,height:22,background:border,margin:"0 5px"}}/>
          <button onClick={()=>setShowFind(f=>!f)} style={{...BStyle(showFind), fontSize:12, padding:"3px 10px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=showFind?hover:"transparent")}>🔍 Find</button>
          <button onClick={()=>setLineNums(l=>!l)} style={{...BStyle(lineNums), fontSize:12, padding:"3px 10px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=lineNums?hover:"transparent")}># Lines</button>
          <button onClick={()=>{ setShowPreview(p=>!p); setShowDiff(false); }} style={{...BStyle(showPreview), fontSize:12, padding:"3px 10px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=showPreview?hover:"transparent")}>📖 Preview</button>
          <button onClick={toggleDiff} style={{...BStyle(showDiff), fontSize:12, padding:"3px 10px"}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=showDiff?hover:"transparent")}>⇄ Diff</button>
        </div>
        ...
        {showFind && (
          <div style={{ display:"flex", alignItems:"center", flexWrap:"wrap", gap:8, padding:"7px 20px", background:darkMode?"#12151f":"#e8e8e8", borderBottom:`1px solid ${border}`, flexShrink:0 }}>
            <div style={{display:"flex",alignItems:"center",gap:5,background:surface,border:`1px solid ${border}`,borderRadius:6,padding:"3px 10px"}}>
              <input placeholder="Find…" value={findText} onChange={e=>setFindText(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter") doFind(); }} style={{border:"none",outline:"none",background:"transparent",color:text_c,fontSize:12,width:140}}/>
              <button onClick={doFind} style={{background:"transparent",border:"none",cursor:"pointer",fontSize:14,color:text_c}}>↓</button>
            </div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:5,background:surface,border:`1px solid ${border}`,borderRadius:6,padding:"3px 10px"}}>
              <input placeholder="Replace with…" value={replaceText} onChange={e=>setReplaceText(e.target.value)} style={{border:"none",outline:"none",background:"transparent",color:text_c,fontSize:12,width:140}}/>
            </div>
        ...
        <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
          <div style={{ flex:1, display:"flex", overflow:"hidden", margin:"16px 20px", gap:12 }}>
            <div ref={editorWrapperRef} style={{ flex:1, display:"flex", flexDirection:"column", background:"#ffffff", border:`1px solid ${border}`, borderRadius:8, overflow:"auto", boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
              <EditorContent editor={editor} className={`tiptap-editor${lineNums?" line-num-gutter":""}`} />
            </div>
            {showPreview && (
              <div style={{ flex:1, background:"#ffffff", border:`1px solid ${border}`, borderRadius:8, overflow:"auto", boxShadow:"0 2px 12px rgba(0,0,0,0.06)", display:"flex", flexDirection:"column" }}>
                ...
                <div className="rich-preview" style={{ flex:1, padding:"20px 24px", fontSize:fontSize, fontFamily: font === "Default" ? "system-ui,sans-serif" : font === "Monospace" ? "'Courier New',monospace" : font, lineHeight:1.8, color:"#1a1a1a", overflow:"auto", pointerEvents:"none", userSelect:"none" }} dangerouslySetInnerHTML={{ __html: editor?.getHTML() ?? "" }} />
              </div>
            )}
            {showDiff && (
              <div style={{ flex:1, background:"#ffffff", border:`1px solid ${border}`, borderRadius:8, overflow:"hidden", boxShadow:"0 2px 12px rgba(0,0,0,0.06)", display:"flex", flexDirection:"column" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 16px", borderBottom:`1px solid ${border}`, background:"#fafafa", borderRadius:"8px 8px 0 0", flexShrink:0 }}>
                  ...
                  <button onClick={() => setDiffSnapshot(editor?.getText() ?? "")} style={{ fontSize:11, padding:"3px 10px", border:`1px solid ${border}`, borderRadius:5, background:"transparent", color:text_c, cursor:"pointer" }} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background="transparent")}>↺ Reset snapshot</button>
                </div>
          ...
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowSpecial(false)}>
            <div onClick={e=>e.stopPropagation()} style={{background:menuBg,border:`1px solid ${border}`,borderRadius:10,padding:24,minWidth:360,boxShadow:"0 16px 40px rgba(0,0,0,0.2)"}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                {SPECIAL.map(c=>(<button key={c} onClick={()=>{ editor?.commands.insertContent(c); setShowSpecial(false); }} style={{width:40,height:40,fontSize:18,background:darkMode?"#12151f":"#f5f5f5",border:`1px solid ${border}`,borderRadius:6,cursor:"pointer",color:text_c}} onMouseEnter={e=>(e.currentTarget.style.background=hover)} onMouseLeave={e=>(e.currentTarget.style.background=darkMode?"#12151f":"#f5f5f5")}>{c}</button>))}
              </div>
        ...
        {showShortcuts && (
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:500,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowShortcuts(false)}>
            <div onClick={e=>e.stopPropagation()} style={{background:menuBg,border:`1px solid ${border}`,borderRadius:10,padding:28,minWidth:360,boxShadow:"0 16px 40px rgba(0,0,0,0.2)",maxHeight:"80vh",overflow:"auto"}}>
              ...
              {[["Ctrl+N","New Tab"],["Ctrl+O","Open File"],["Ctrl+S","Save .html"],["Ctrl+Shift+S","Save .txt"],["Ctrl+P","Print"],["Ctrl+Z","Undo"],["Ctrl+Y","Redo"],["Ctrl+B","Bold"],["Ctrl+I","Italic"],["Ctrl+F","Find & Replace"],["Ctrl+M","Rich Preview"],["Ctrl+D","Diff View"]].map(([k,d])=>(
  ...
  return (
    <div>
      ...
      {diff.map((line, idx) => {
        ...
        return (
          <div key={idx} style={{ display:"flex", alignItems:"stretch", background: isAdd ? "rgba(67,160,71,0.10)" : isDel ? "rgba(229,57,53,0.10)" : "transparent", borderLeft: isAdd ? "3px solid #43a047" : isDel ? "3px solid #e53935" : "3px solid transparent" }}>
            <div style={{ display:"flex", flexShrink:0, userSelect:"none" }}>
              <span style={{ width:40, textAlign:"right", padding:"1px 8px", color:"#bbb", fontSize:11, lineHeight:"1.7" }}>{line.origLine ?? ""}</span>
              <span style={{ width:40, textAlign:"right", padding:"1px 8px", color:"#bbb", fontSize:11, lineHeight:"1.7", borderRight:"1px solid #eee" }}>{line.modLine ?? ""}</span>
            </div>
            <span style={{ padding:"1px 6px", color: isAdd ? "#43a047" : isDel ? "#e53935" : "#ccc", fontWeight:700, fontSize:12, lineHeight:"1.7", flexShrink:0 }}>
              {isAdd ? "+" : isDel ? "−" : " "}
            </span>
            <span style={{ padding:"1px 8px 1px 2px", whiteSpace:"pre-wrap", wordBreak:"break-all", color: isAdd ? "#2e7d32" : isDel ? "#c62828" : "#333", fontSize:13, lineHeight:"1.7", flex:1 }}>
              {line.text || " "}
            </span>
          </div>
        );
