...
/* ── CSS for exports with proper spacing ─────────────────────────────────── */
function buildExportCSS(): string {
  return `
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h1 { font-size: 2em;    font-weight: 700; margin: 0.8em 0 0.4em;  line-height: 1.2;  color: inherit; page-break-after: avoid; }
    h2 { font-size: 1.5em;  font-weight: 600; margin: 0.75em 0 0.35em; line-height: 1.3; color: inherit; page-break-after: avoid; }
    h3 { font-size: 1.25em; font-weight: 600; margin: 0.65em 0 0.3em;  color: inherit; page-break-after: avoid; }
    h4, h5, h6 { font-size: 1em; font-weight: 600; margin: 0.6em 0 0.25em; color: inherit; page-break-after: avoid; }
    ...
    @page { margin: ${MARGIN_MM}mm; size: auto; }
  `;
}
...
function prepareHtml(rawHtml: string): string {
  ...
  doc
    .querySelectorAll(".col-resizer,.table-resizer-wrap,.column-resize-handle")
...
/* ── PDF Export with Proper Margins ──────────────────────────────────────── */
export async function exportToPDF(
  ...
  // Calculate content area (page minus margins on both sides)
  const MARGIN_PX = mmToPx(MARGIN_MM);
  const CONTENT_W_PX = PAGE_W_PX - (MARGIN_PX * 2);
  const CONTENT_H_PX = PAGE_H_PX - (MARGIN_PX * 2);
   ...
   * The padding inside the CSS will create the margins.
   */
  ...
  // Add content without additional CSS padding (container padding handles it)
  ...
  style.textContent = buildExportCSS().replace(`padding: ${MARGIN_MM}mm;`, 'padding: 0;'); // Remove body padding since container has it
  ...
  try {
    ...
    const fullCanvas = await html2canvas(container, {
      ...
      windowHeight: Math.max(PAGE_H_PX, container.scrollHeight),
    });

    const pxPerMm = fullCanvas.width / PAGE_W_MM;
    const pageHcanvas = PAGE_H_MM * pxPerMm;
    ...
    for (let p = 0; p < totalPages; p++) {
      ...
      pc.height = Math.round(pageHcanvas);
      ...
      // Add image to PDF at full page size (margins are already in the canvas)
    ...
    sections: [
      {
        properties: {
          page: {
            margin: {
              top: Math.round(MARGIN_MM * MM_TO_TWIP),
              right: Math.round(MARGIN_MM * MM_TO_TWIP),
              bottom: Math.round(MARGIN_MM * MM_TO_TWIP),
              left: Math.round(MARGIN_MM * MM_TO_TWIP),
            },
          },
        },
